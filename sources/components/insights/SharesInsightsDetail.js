...
const SharesInsightsDetail = ({ insight, ...props }) => {
  ...
  const data = useMemo(() => {
    return {
      ...
      slices: [
        { title: languageContent.venue_share },
        { title: languageContent.event_share },
      ],
    };
  }, []);
        ...
        <MainPieChart data={data}>
          <MainText align="center" font="subtitle-1">
            {business_count}{" "}
            {
              languageContent[
                business_count == 1
                  ? "person_share_business"
                  : "people_share_business"
              ]
            }{" "}
            {event_count}{" "}
            {
              languageContent[
                event_count == 1 ? "person_share_event" : "people_share_event"
              ]
            }
          </MainText>
        </MainPieChart>
...
export default memo(SharesInsightsDetail);
