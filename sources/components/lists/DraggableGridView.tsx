...
const GridView = (props: GridViewProps) => {
  ...
  const delayLongPress = rest.delayLongPress || 100;
  ...
  const animate = useCallback(() => {
    ...
    const sy = cellSize.height / 2;
    let a = 0;
    if (move < top + sy) {
      a = Math.max(-sy, move - (top + sy)); // above
    } else if (move > frame.height - bottom - sy) {
      a = Math.min(sy, move - (frame.height - bottom - sy)); // below
    }
    a && scroll((a / sy) * 10); // scrolling
  ...
  return (
    <ScrollView
      ...
      contentContainerStyle={{
        ...rest.containerMargin,
      }}
    >
      <View
        style={{
          height: top + self.numRows * self.cellSize.height + bottom,
        }}
      />
